...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    ]).then(([casesData, doctorsData, finData, statsData]) => {
      setCases(casesData);
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-zinc-100">
                {cases.slice(0, 5).map((c) => (
                  <tr key={c.id} className="hover:bg-zinc-50 transition-colors cursor-pointer" onClick={() => window.location.href = `/cases/${c.id}`}>
