...
export default function DoctorPortal() {
  ...
  const [balance, setBalance] = useState({ total_invoiced: 0, total_paid: 0, outstanding_balance: 0 });
  ...
  useEffect(() => {
    const savedDoctor = localStorage.getItem("doctor_user");
    if (savedDoctor) {
      setLoggedInDoctor(JSON.parse(savedDoctor));
    }
  }, []);

  useEffect(() => {
    if (loggedInDoctor) {
      fetchDoctorData(loggedInDoctor.doctor_id);
    }
  }, [loggedInDoctor]);
  ...
  const fetchDoctorData = async (doctorId: number) => {
    setDataLoading(true);
    try {
      const res = await fetch(`/api/doctors/${doctorId}/portal-data?startDate=${dateFilter.start}&endDate=${dateFilter.end}`);
      if (res.ok) {
        ...
        {!loggedInDoctor ? (
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {dataLoading && balance.total_invoiced === 0 ? (
                [1, 2, 3].map(i => (
              ...
              <div className="space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  <div className="bg-white p-4 rounded-2xl border border-zinc-100 shadow-sm">
                    ...
                    <p className="text-xl font-black text-amber-500">{cases.filter(c => c.status === 'Pending').length}</p>
                  </div>
                  <div className="bg-white p-4 rounded-2xl border border-zinc-100 shadow-sm">
                    <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-1">In Progress</p>
                    <p className="text-xl font-black text-blue-500">{cases.filter(c => c.status === 'In Progress').length}</p>
                  </div>
                  <div className="bg-white p-4 rounded-2xl border border-zinc-100 shadow-sm">
                    <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-1">Trial</p>
                    <p className="text-xl font-black text-indigo-500">{cases.filter(c => c.status === 'Trial').length}</p>
                  </div>
                  <div className="bg-white p-4 rounded-2xl border border-zinc-100 shadow-sm">
                    <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-1">Completed</p>
                    <p className="text-xl font-black text-emerald-500">{cases.filter(c => c.status === 'Completed').length}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="bg-white rounded-3xl border border-zinc-100 shadow-sm divide-y divide-zinc-50">
                      ...
                      {cases.filter(c => c.status !== 'Completed' && c.status !== 'Delivered').length === 0 && (
            ...
            {activeTab === 'cases' ? (
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <h2 className="text-xl font-bold text-zinc-900">Your Cases ({cases.length})</h2>
                  <Link 
                    to="/new-case" 
                    ...
                    cases.map((c) => (
                      ...
                      >
                        <div className="flex justify-between items-start mb-4">
                          <div>
                            <p className="text-xs font-bold text-zinc-400 uppercase tracking-widest mb-1">Case #{c.id.toString().padStart(4, '0')}</p>
