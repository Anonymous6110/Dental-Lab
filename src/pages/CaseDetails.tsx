  ...
  const fetchData = async () => {
    try {
      const [caseRes, historyRes, tasksRes, techRes] = await Promise.all([
        ...
        fetch(`/api/cases/${id}/history`).then(res => res.json()),
        fetch(`/api/cases/${id}/tasks`).then(res => res.json()),
        fetch(`/api/technicians`).then(res => res.json())
      ]);
      setDentalCase(caseRes);
      setHistory(historyRes);
      setTasks(tasksRes);
      setTechnicians(techRes);
      setUpdateData({ status: caseRes.status, comment: "" });
      setLoading(false);
    } catch (err) {
    ...
    <div className="space-y-8">
      <header className="flex items-center justify-between">
        <div className="flex items-center">
          <button onClick={() => navigate("/cases")} className="p-2 hover:bg-zinc-100 rounded-full mr-4 transition-colors">
            <ChevronLeft size={24} />
          </button>
          <div>
            ...
            <p className="text-zinc-500 mt-1">Case #{dentalCase.id.toString().padStart(4, '0')} • {dentalCase.doctor_name}</p>
          </div>
        </div>
